/* 
Template name: Project three
*/
<?php wp_header(); ?>
    <div class="container projects_one">
        <h1 class="text-center mb-4">CRM aplication</h1>
        <dt class="mb-4">Customer management system with login, user roles, search/sort/filter, responsive UI, built using PHP MVC + Bootstrap.

        This project is a full-featured CRM system built using PHP, MySQL, JavaScript, and Bootstrap, designed to help businesses manage customer data, interactions, and internal workflows. The application follows the MVC architecture to ensure separation of concerns and clean code structure.

        The system allows users to create, read, update, and delete customer records, track communication history, and manage user access levels. All operations are validated both on the frontend (JavaScript) and backend (PHP), with secure database interactions using PDO.

        Key Features:
        Customer Management: Add, edit, delete, and search customers

        User Authentication: Login system with role-based access (e.g., admin, employee)

        Dashboard: Overview of recent activity and quick access to customer data

        Validation & Security: Input validation, session management, and secure queries

        Search & Filters: Fast filtering and searching through customer database

        Responsive UI: Clean, mobile-friendly layout using Bootstrap

        MVC Structure: Organized codebase for scalability and maintainability
        </dt>
        <div class="row">
            <div class="col-md-4">
                <div class="project-images">
                    <img src="img/about_me.jpg" class="img-fluid" alt="Project image 1" />
                </div>
            </div>
            <div class="col-md-4">
                <div class="project-images">
                    <img src="img/about.jpg" class="img-fluid" alt="Project image 1" />
                </div>
            </div>
            <div class="col-md-4">
                <div class="project-images">
                    <img src="img/about.jpg" class="img-fluid" alt="Project image 1" />
                </div>
            </div>
        </div>
        </br>
        <div class="row">
            <div class="col-md-4">
                <div class="project-images">
                    <img src="img/about_me.jpg" class="img-fluid" alt="Project image 1" />
                </div>
            </div>
            <div class="col-md-4">
                <div class="project-images">
                    <img src="img/about.jpg" class="img-fluid" alt="Project image 1" />
                </div>
            </div>
            <div class="col-md-4">
                <div class="project-images">
                    <img src="img/about.jpg" class="img-fluid" alt="Project image 1" />
                </div>
            </div>
            <a href="index.html#projects" class="back-link nav-link mt-4">← Back to Projects</a>
        </div>
    </div>
<?php wp_footer(); ?>